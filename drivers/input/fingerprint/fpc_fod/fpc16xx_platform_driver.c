...
static ssize_t power_ctrl_set(struct device *dev, struct device_attribute *attr,
...
static struct attribute *fpc16xx_attrs[] = { &dev_attr_power_ctrl.attr,
...
static void fpc_screen_state_for_fingerprint_callback(
	...
	struct panel_event_notification *notification, void *client_data)
{
	...
	if (notifier_tag == PANEL_EVENT_NOTIFICATION_PRIMARY) {
		...
		case DRM_PANEL_EVENT_UNBLANK:
			...
			mutex_unlock(&fpc16xx->fb_lock);
			sysfs_notify(&fpc16xx->dev->kobj, NULL,
...
static void fpc_register_panel_notifier_work(struct work_struct *work)
{
	...
	struct fpc16xx_data *fpc16xx =
		container_of(work, struct fpc16xx_data, screen_state_dw.work);
	...
	if (active_panel) {
		...
		if (!cookie) {
			cookie = panel_event_notifier_register(
				...
				active_panel,
				fpc_screen_state_for_fingerprint_callback,
				(void *)fpc16xx);
			if (IS_ERR(cookie))
		...
		if (retry_count++ < 5) {
			pr_err("Failed to register panel notifier %d times, try again 5s later",
			       retry_count);
			queue_delayed_work(fpc16xx->screen_state_wq,
					   &fpc16xx->screen_state_dw,
...
#ifdef FPC_ENABLE_SCREEN_MONITOR
	fpc16xx->screen_state_wq =
		create_singlethread_workqueue("screen_state_wq");
	if (fpc16xx->screen_state_wq) {
		INIT_DELAYED_WORK(&fpc16xx->screen_state_dw,
				  fpc_register_panel_notifier_work);
