	...
	if (!strcmp(pwrkey->data->name, "pmic_pwrkey_bark")) {
		...
		if (sts) {
			if (mi_display_pwrkey_cb != NULL) {
				mi_display_pwrkey_cb(
					PMIC_PWRKEY_CLOSE_BRIGHNESS);
			}
			dev_err(pwrkey->dev,
				"pwrkey_bark_irq trigger, call notifier chain to start mtdoops dump");
			atomic_notifier_call_chain(&pwrkey_irq_notifier_list, 0,
						   NULL);
		}
		return IRQ_HANDLED;
